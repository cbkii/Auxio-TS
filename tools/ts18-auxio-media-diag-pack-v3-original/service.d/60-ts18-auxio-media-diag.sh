...
###############################################################################
# Attempt Auxio in-app diagnostic session if present
###############################################################################

for pkg in $PKGS; do
  safe="$(safe_name "$pkg")"
  pd="$OUT/packages/$safe/dumpsys-package.txt"
  [ -f "$pd" ] || continue
  if grep -q 'org.oxycblt.auxio.diagnostics.DiagnosticService' "$pd" || grep -q 'DiagnosticService' "$pd"; then
    out="$OUT/auxio/start-diagnostic-$safe.txt"
    action="$pkg.diagnostics.START_CAPTURE"
    {
      echo "### Start Auxio in-app DiagnosticService"
      echo "# Package: $pkg"
      echo "# Action: $action"
      echo "# Time: $(human_ts)"
      echo
      for cls in ".diagnostics.DiagnosticService" "org.oxycblt.auxio.diagnostics.DiagnosticService" "$pkg.diagnostics.DiagnosticService"; do
        comp="$pkg/$cls"
        echo "Trying: $comp"
        am start-foreground-service -n "$comp" -a "$action" --es extra_session_id "$SESSION_ID" --el extra_duration_ms "$AUXIO_DIAG_DURATION_MS" --es extra_origin "magisk_service_d" 2>&1
        echo "exit=$?"
        echo
      done
    } > "$out"
...

{
  ...
  echo "| Auxio in-app diagnostics | auxio/logcat | $(feature_status a 'DiagnosticService|DiagnosticJournal|Capture Service' "$OUT/logs/logcat_filtered.txt") | auxio/start-diagnostic-*.txt |"
