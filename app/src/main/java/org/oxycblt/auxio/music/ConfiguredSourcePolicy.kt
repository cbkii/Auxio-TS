...
/**
 * Produces immutable, operation-scoped snapshots of the user's configured music-source policy.
 *
 * The snapshot is deliberately separate from source discovery: a discovered removable volume can
 * be offered to the user, but it cannot become a runtime source or Fast Start root until it is
 * represented in a later configured snapshot.
 */
@Singleton
class ConfiguredSourcePolicy @Inject constructor(private val settings: MusicSettings) {
    ...
    /** Unconfigured removable discovery remains explicit-user-action only. */
