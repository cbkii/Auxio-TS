...
class QueueAuthorityPolicyTest {
    @Test
    fun acceptsOnlyTheCurrentPrimitiveGenerationAndDescriptor() {
        ...
        assertTrue(
            QueueAuthorityPolicy.accepts(
                ...
                resultItems = listOf(item(uri = "file:///storage/emulated/0/Music/a.mp3")),
            )
        )
    }
    ...
    fun rejectsSyntheticMissingRows() {
        ...
        assertFalse(
            QueueAuthorityPolicy.accepts(
                ...
                activeDescriptor = descriptor,
                resultDescriptor = descriptor,
                resultItems = listOf(item(uri = null)),
            )
        )
        assertTrue(QueueAuthorityPolicy.hasMissingRows(listOf(item(uri = null))))
    }
    ...
    private fun item(uri: String?, stableUid: String? = null) =
        QueueItemRef(
            logicalPosition = 0,
            canonicalPosition = 0,
