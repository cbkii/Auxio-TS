...
class TopwayLauncherIntegrationCoordinatorTest {
    ...
    fun `progress rate limit suppresses periodic duplicate but force bypasses`() {
        ...
        assertEquals(
            2,
            context.broadcasts.countDofunTargeted(TopwayMusicContract.ACTION_PROGRESS_DURATION),
        )
    }
    ...
    fun `mode gates outgoing broadcasts`() {
        ...
        assertEquals(
            1,
            broadcastContext.broadcasts.countImplicit(TopwayMusicContract.ACTION_PROGRESS_DURATION),
        )
        assertEquals(
            1,
            broadcastContext.broadcasts.countDofunTargeted(
                TopwayMusicContract.ACTION_PROGRESS_DURATION
            ),
        )
    }
    ...
    private fun List<Intent>.countImplicit(action: String): Int = count {
        it.action == action && it.`package` == null
    }

    private fun List<Intent>.countDofunTargeted(action: String): Int = count {
        it.action == action && it.`package` == DOFUN_PACKAGE
    }
