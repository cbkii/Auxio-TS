...
class NotificationBitmapSafetyTest {
    @Test fun fallbackBitmapIsTs18SafeSizeAndCached() {
    ...
    @Test fun extremeAspectRatioArtworkIsPaddedWithoutDistortion() {
        val sanitized = NotificationBitmapSafety.sanitize(Bitmap.createBitmap(100, 2048, Bitmap.Config.ARGB_8888))
        assertNotNull(sanitized)
        assertEquals(NotificationBitmapSafety.FALLBACK_ICON_SIZE_PX, sanitized!!.width)
        assertEquals(NotificationBitmapSafety.FALLBACK_ICON_SIZE_PX, sanitized.height)
    }
