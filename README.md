...
#### 🚘 Landscape optimised, large touch controls.
#### ⏯️ Floating playback controls and optional startup actions **[beta]**.
#### 💾 Choose Direct file access, Android's file picker or the Android system library.
#### ⚡ Faster launch and fast-resume (cached library, queue and playback state after relaunch or restart).
#### 🔊 MediaSession, MediaBrowser, audio focus and media button. Topway and DoFun compatibility paths **[beta]**.
...
> [!IMPORTANT]
> On a rooted TS18, grant superuser access *before* selecting a Direct source. This avoids starting Direct scan without root-assisted storage access enabled.
...
| --- | --- | --- |
| **Direct** | TS18 USB drives and known folders | Recommended for paths such as `/storage/usbdisk0/*`. |
...
- ⚙️ [Settings guide](docs/SETTINGS_GUIDE.md) 
...
> [!TIP]
> ### 🧰 Common problems
> When reporting a problem, include the Auxio-TS version, package name, TS18 build, selected source mode and the exact path used.
> Do not upload private files, credentials or personal data.
> 
