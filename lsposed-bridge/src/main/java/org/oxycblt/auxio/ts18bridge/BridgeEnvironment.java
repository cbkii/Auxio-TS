...
final class BridgeEnvironment {
    ...
    enum KillSwitchState { ENABLED, DISABLED, UNKNOWN }

    private static final long CACHE_MS = 3_000L;
    private final LogSink log;
    private final ExecutorService probe = Executors.newSingleThreadExecutor(task -> {
    ...
    BridgeEnvironment(LogSink log) { this.log = log; }
    ...
    Context currentContext() { return context.get().get(); }
    ...
    boolean canPublish(Context value) { return canBridge(value, "publish"); }
    ...
    void refreshAsync(Context value, Consumer<Boolean> completion) {
        ...
        if (app == null) { complete(completion, false); return; }
    ...
    private State inspect(Context value) {
        ...
        return new State(true, killSwitch, identity.identity, identity.functional,
                identity.entry, targetReady, identity.versionCode, now);
    }
    ...
    private Identity inspectStock(Context value) {
        try {
            PackageInfo info = value.getPackageManager().getPackageInfo(
                    BridgeContract.STOCK_PACKAGE, PackageManager.GET_SIGNING_CERTIFICATES);
            ...
            String expected = normalise(BuildConfig.STOCK_CERT_SHA256);
            if (!isDigest(expected) || !matchesCurrentSigner(info.signingInfo, expected)) {
                return Identity.untrusted(version);
            }
            BridgeContract.RegistryEntry entry =
                    BridgeContract.reviewedStockApk(apkDigest(info.applicationInfo.sourceDir));
            boolean functional = entry != null && expected.equals(entry.signerSha256);
    ...
    private boolean inspectTarget(PackageManager manager) {
        try {
            ...
            if (!manager.getActivityInfo(component(BuildConfig.TARGET_ACTIVITY), 0).exported) return false;
            if (!manager.getServiceInfo(component(BuildConfig.TARGET_MEDIA_BROWSER_SERVICE), 0).exported) return false;
    ...
    private static boolean matchesCurrentSigner(SigningInfo info, String expected) {
        ...
        for (Signature signer : signers) if (expected.equals(sha256(signer.toByteArray()))) return true;
        return false;
    }
    ...
    private KillSwitchState readKillSwitch() {
        try {
            ...
            if (shared == null || !shared.isDirectory() || !shared.canRead()) return KillSwitchState.UNKNOWN;
            String storageState = Environment.getExternalStorageState(shared);
            if (!Environment.MEDIA_MOUNTED.equals(storageState)
                    && !Environment.MEDIA_MOUNTED_READ_ONLY.equals(storageState)) return KillSwitchState.UNKNOWN;
            File parent = new File(shared, "Auxio-TS");
            if (parent.exists() && (!parent.isDirectory() || !parent.canRead())) return KillSwitchState.UNKNOWN;
            File marker = new File(parent, "disable-lsposed-bridge");
            if (marker.exists()) return marker.isFile() ? KillSwitchState.DISABLED : KillSwitchState.UNKNOWN;
            return parent.exists() || shared.canRead() ? KillSwitchState.ENABLED : KillSwitchState.UNKNOWN;
        } catch (RuntimeException error) {
            return KillSwitchState.UNKNOWN;
        }
    }
    ...
    private String apkDigest(String path) {
        ...
        if (!digest.isEmpty()) { digestKey = key; digestValue = digest; }
        return digest;
    }
    ...
    private void logState(State value) {
        if (!value.identityTrusted) {
            ...
            log.log("stock identity verified but APK hash is not reviewed; functional hooks inactive", null);
    ...
    private static boolean isDigest(String value) { return value.matches("[0-9A-F]{64}"); }

    private void complete(Consumer<Boolean> completion, boolean value) {
        if (completion == null) return;
        try { completion.accept(value); }
        catch (RuntimeException error) { log.log("bridge readiness completion failed safely", error); }
    }
    ...
    private static final class Identity {
        ...
        Identity(boolean identity, boolean functional, BridgeContract.RegistryEntry entry, long versionCode) {
            this.identity = identity; this.functional = functional; this.entry = entry; this.versionCode = versionCode;
        }
        static Identity untrusted(long version) { return new Identity(false, false, null, version); }
    }
    ...
    private static final class State {
        ...
        State(boolean known, KillSwitchState killSwitch, boolean identityTrusted,
                boolean functionalIdentityTrusted, BridgeContract.RegistryEntry entry,
                boolean targetReady, long versionCode, long checkedAtMs) {
            this.known = known; this.killSwitch = killSwitch; this.identityTrusted = identityTrusted;
            this.functionalIdentityTrusted = functionalIdentityTrusted; this.entry = entry;
            this.targetReady = targetReady; this.versionCode = versionCode; this.checkedAtMs = checkedAtMs;
        }
        static State unknown() {
            return new State(false, KillSwitchState.UNKNOWN, false, false, null, false, 0L, 0L);
        }
        boolean canBridge() {
            return known && functionalIdentityTrusted && killSwitch == KillSwitchState.ENABLED && targetReady;
        }
    }
}
