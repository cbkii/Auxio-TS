...
final class MediaMirror {
    interface LogSink { void log(String message, Throwable error); }
    ...
    private final ThreadPoolExecutor commandExecutor = new ThreadPoolExecutor(
    ...
    private final ServiceConnection commandConnection = new ServiceConnection() {
        @Override public void onServiceConnected(ComponentName name, IBinder service) {
            clearCommandBinder();
            if (stopped.get() || commandCircuitOpen.get()) { disconnectCommandService(); return; }
            try {
                service.linkToDeath(commandDeathRecipient, 0);
                commandBinder = service;
            } catch (RemoteException | RuntimeException error) {
                commandEndpointFailed("command Binder died during connection", error);
            }
        }
    ...
    private final Runnable progressTick = new Runnable() {
        @Override public void run() {
            ...
            publishProgress();
            if (isPlaying(controller.getPlaybackState())) handler.postDelayed(this, TICK_MS);
        }
    };
    ...
    private final MediaBrowser.ConnectionCallback browserCallback = new MediaBrowser.ConnectionCallback() {
    ...
    private void startOrRetryOnHandler() {
        ...
        if (started.compareAndSet(false, true)) {
            reconnectAttempts = 0;
            connect();
        } else if (!browserConnectionPending && controller == null
    ...
    boolean dispatchCommand(BridgeCommand command, Integer seekPosition) {
        if (command == BridgeCommand.UNKNOWN || stopped.get() || !environment.canPublish(context)) return false;
        if (command == BridgeCommand.UPDATE) return publishSnapshot();

        if (Looper.myLooper() == Looper.getMainLooper()) {
            if (loggedMainThreadFallback.compareAndSet(false, true)) {
                log.log("command hook is on stock main looper; command suppression remains fail-open", null);
            }
            return false;
        }
        ...
        if (commandCode == 0 || (command == BridgeCommand.SEEK && seekPosition == null)) return false;
        long seekMs = command == BridgeCommand.SEEK ? Math.max(0L, seekPosition.longValue()) : -1L;
        IBinder target = commandBinder;
        if (target == null || !target.isBinderAlive()) { startOrRetry(); return false; }
        ...
        try {
            ...
            correlation.complete(decision, false, SystemClock.elapsedRealtime());
            future.cancel(true);
            tripCircuit("command Binder timed out; suppression disabled for this stock process", error);
            return false;
            ...
            correlation.complete(decision, false, SystemClock.elapsedRealtime());
            handler.post(() -> commandEndpointFailed("command Binder execution failed", error.getCause()));
            return false;
        }
    }

    private int transact(IBinder target, long commandId, int commandCode, long seekMs, long createdAtMs)
            throws RemoteException {
        if (commandCircuitOpen.get() || !target.isBinderAlive()) return BridgeWireContract.RESULT_NOT_READY;
    ...
    private void tripCircuit(String message, Throwable error) {
        ...
        commandExecutor.shutdownNow();
        handler.post(this::disconnectCommandService);
    }

    private void connect() {
        if (stopped.get() || !environment.canPublish(context)) { started.set(false); return; }
        disconnectBrowser();
        try {
            browserConnectionPending = true;
            browser = new MediaBrowser(context,
                    new ComponentName(BuildConfig.TARGET_PACKAGE, BuildConfig.TARGET_MEDIA_BROWSER_SERVICE),
    ...
    private void connectCommandService() {
        if (stopped.get() || commandCircuitOpen.get() || commandBindRequested || commandBinder != null
                || !environment.canPublish(context)) return;
        Intent intent = new Intent(BridgeWireContract.ACTION_BIND_COMMAND)
                .setComponent(new ComponentName(BuildConfig.TARGET_PACKAGE, BuildConfig.TARGET_MEDIA_BROWSER_SERVICE));
        try {
            commandBindRequested = context.bindService(intent, commandConnection, Context.BIND_AUTO_CREATE);
            if (!commandBindRequested) log.log("command endpoint bind rejected; stock path retained", null);
        } catch (RuntimeException error) {
    ...
    private void commandEndpointFailed(String message, Throwable error) {
        log.log(message, error);
        disconnectCommandService();
        if (!stopped.get() && !commandCircuitOpen.get()) scheduleReconnect();
    }
    ...
    private void disconnectCommandService() {
        ...
        commandBindRequested = false;
        try { context.unbindService(commandConnection); }
    ...
    private void clearCommandBinder() {
        ...
        commandBinder = null;
        if (current != null) try { current.unlinkToDeath(commandDeathRecipient, 0); }
    ...
    private void clearController() {
        ...
        controller = null;
        if (current != null) try { current.unregisterCallback(controllerCallback); }
        catch (RuntimeException ignored) { }
    }
    ...
    private boolean canPublish() { return !stopped.get() && environment.canPublish(context); }

    private boolean publishMetadata(MediaMetadata metadata) {
        if (!canPublish()) return false;
        String title = "", artist = "", album = "", mediaUri = "";
        long duration = 0L;
        if (metadata != null) {
            CharSequence displayTitle = metadata.getText(MediaMetadata.METADATA_KEY_DISPLAY_TITLE);
            title = boundedText(displayTitle != null ? displayTitle : metadata.getText(MediaMetadata.METADATA_KEY_TITLE));
        ...
        try {
            context.sendBroadcast(new Intent(ACTION_MUSIC_INFO)
                    .putExtra("musicTitle", title).putExtra("musicaArtist", artist)
                    .putExtra("musicAlbum", album).putExtra("musicPath", mediaUri));
            context.sendBroadcast(new Intent(ACTION_LEGACY_METADATA)
                    .putExtra("track", title).putExtra("artist", artist).putExtra("album", album)
                    .putExtra("duration", duration).putExtra("playing", playing)
                    .putExtra("package", BridgeContract.STOCK_PACKAGE));
            return true;
        } catch (RuntimeException error) { log.log("metadata publish failed", error); return false; }
    }

    private boolean publishPlayState(PlaybackState state) {
        if (!canPublish()) return false;
        try {
            context.sendBroadcast(new Intent(ACTION_LEGACY_PLAYSTATE)
                    .putExtra("playing", isPlaying(state)).putExtra("package", BridgeContract.STOCK_PACKAGE));
            return true;
        } catch (RuntimeException error) { log.log("play-state publish failed", error); return false; }
    }
    ...
    private boolean publishProgress() {
        ...
        long duration = metadata != null ? Math.max(0L, metadata.getLong(MediaMetadata.METADATA_KEY_DURATION)) : 0L;
        long position = calculatedPosition(state, duration);
        try {
            context.sendBroadcast(new Intent(ACTION_PROGRESS)
                    .putExtra("msg_music_progress", boundedInt(position))
                    .putExtra("msg_music_duration", boundedInt(duration)));
            return true;
        } catch (RuntimeException error) { log.log("progress publish failed", error); return false; }
    }
    ...
    private static long calculatedPosition(PlaybackState state, long duration) {
        ...
        if (isPlaying(state) && state.getLastPositionUpdateTime() > 0L) {
            position += (long) (Math.max(0L, SystemClock.elapsedRealtime() - state.getLastPositionUpdateTime())
                    * state.getPlaybackSpeed());
        }
        return duration > 0L ? Math.max(0L, Math.min(position, duration)) : Math.max(0L, position);
    }

    private static int boundedInt(long value) { return (int) Math.max(0L, Math.min(value, Integer.MAX_VALUE)); }

    private static String boundedText(CharSequence value) {
        if (value == null) return "";
        String text = value.toString();
        return text.length() <= MAX_METADATA_CHARS ? text : text.substring(0, MAX_METADATA_CHARS);
    }

    private static long createClientGeneration() {
        long value = SystemClock.elapsedRealtimeNanos() ^ System.nanoTime() ^ (((long) Process.myPid()) << 32);
