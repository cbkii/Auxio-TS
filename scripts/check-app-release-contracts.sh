#!/usr/bin/env bash
# Validate a staged signed Auxio application/add-on APK before publication.
set -euo pipefail
...
case "$expected_package" in org.oxycblt.auxio|com.tw.media|com.tw.music|org.oxycblt.auxio.ts18bridge) ;; *) fail "Unsupported release application ID: ${expected_package:-<unset>}" ;; esac
...
if [[ "$expected_package" != org.oxycblt.auxio.ts18bridge ]]; then
  ...
  [[ "$actual_min_sdk" =~ ^[0-9]+$ && "$actual_min_sdk" -le 29 ]] || fail "LSPosed bridge must remain installable on API 29; minSdk is ${actual_min_sdk:-<empty>}."
fi
...
for expected_metadata in "application_id=${actual_package}" "version_code=${actual_version_code}" "version_name=${actual_version_name}" "min_sdk=${actual_min_sdk}" "target_sdk=${actual_target_sdk}" "abis=${actual_abis}"; do
...
checksum_dir=$(cd -- "$(dirname -- "$sha256_file")" && pwd -P); checksum_name=$(basename -- "$sha256_file")
apk_dir=$(cd -- "$(dirname -- "$apk")" && pwd -P); apk_name=$(basename -- "$apk")
...
printf 'Signed Auxio asset contracts: PASS (%s)\n' "$expected_package"
