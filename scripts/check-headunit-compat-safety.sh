...
allowed_stock_wrapper='app/src/topwayCompat/java/com/tw/music/'
...
product_sources=()
for path in app/src/main app/src/topwayCompat app/src/test app/src/androidTest musikr/src; do
...
identity_files=()
for path in app/build.gradle "${manifest_path}" app/src/main/res/values/donottranslate.xml app/src/debug/res/values/donottranslate.xml musikr/build.gradle settings.gradle; do
...
# Auxio-TS has one application identity. Stock-compatible class names are component contracts,
# never permission to produce an installable com.tw.music application.
...
if [ -n "${identity_hits}" ]; then
  while IFS= read -r line; do
    ...
    case "${path}" in
      app/build.gradle)
        if grep -Fq 'applicationId "com.tw.media"' app/build.gradle && ! grep -Fq 'applicationId "com.tw.music"' app/build.gradle; then
          continue
        fi
        ;;
    ...
    echo "${line}" >&2
    echo "Unexpected application identity outside the maintained com.tw.media product" >&2
    exit 1
  done <<< "${identity_hits}"
fi
  ...
  while IFS= read -r line; do
    ...
    case "${path}" in
      ...
      "${root_probe_allowlist_test}")
        ;;
      ${allowed_topway_main}*|${allowed_topway_test}*|${allowed_stock_wrapper}*)
        case "${line}" in
...
if [ -f "${compat_manifest}" ]; then
  require_topway_identity() {
    local pattern="$1"
    local label="$2"
    if ! grep -Fq "${pattern}" "${compat_manifest}"; then
      echo "Missing ${label} in ${compat_manifest}: ${pattern}" >&2
      exit 1
    fi
  }
  ...
  require_topway_identity 'android:name=".AuxioService"' 'Auxio playback service'
  require_topway_identity 'org.oxycblt.auxio.car.overlay.ACTION_RESTORE_OVERLAY' 'overlay restore action'
  ...
  if ! grep -Fq 'android:foregroundServiceType="specialUse"' "${compat_manifest}"; then
    echo "Topway overlay manifest must declare foregroundServiceType=\"specialUse\" for API 34+ forward compatibility" >&2
    exit 1
  fi
  if ! grep -Fq 'FOREGROUND_SERVICE_SPECIAL_USE' "${compat_manifest}"; then
...
if [ -f app/src/main/java/org/oxycblt/auxio/headunit/overlay/CarOverlayContract.kt ] && [ -f "${compat_manifest}" ]; then
