#!/system/bin/sh
# TS18 DoFun/TW Music/Auxio runtime validation collector v2
# Safe/read-only by default. Designed for TermOnePlus/TermOne without root or ADB.
# Fixes v1 probe issue by using --user 0 for am/pm/cmd user-scoped calls.
...
BASE="/sdcard/Download/ts18_dofun_runtime_validation_v2_${TS}"
...
This collector is safe/read-only by default. It uses explicit --user $USER_ID for am/pm/cmd probes to avoid the user -2 failure seen in v1.
