...
class MediaLibraryService2LegacyStub extends MediaBrowserServiceCompat {
    ...
    public MediaBrowserServiceCompat.BrowserRoot onGetRoot(String str, int i, Bundle bundle) {
        ...
        MediaLibraryService2.LibraryRoot libraryRootOnGetLibraryRoot = this.mLibrarySession.getCallback().onGetLibraryRoot(this.mLibrarySession.getInstance(), getController(), bundle);
        if (libraryRootOnGetLibraryRoot == null) {
            return null;
        }
        return new MediaBrowserServiceCompat.BrowserRoot(libraryRootOnGetLibraryRoot.getRootId(), libraryRootOnGetLibraryRoot.getExtras());
    }
    ...
    public void onLoadItem(final String str, final MediaBrowserServiceCompat.Result<MediaBrowserCompat.MediaItem> result) {
        ...
        this.mLibrarySession.getCallbackExecutor().execute(new Runnable() { // from class: android.support.v4.media.MediaLibraryService2LegacyStub.2
            ...
            public void run() {
                MediaItem2 mediaItem2OnGetItem = MediaLibraryService2LegacyStub.this.mLibrarySession.getCallback().onGetItem(MediaLibraryService2LegacyStub.this.mLibrarySession.getInstance(), controller, str);
                if (mediaItem2OnGetItem == null) {
                    result.sendResult(null);
                } else {
                    result.sendResult(MediaUtils2.convertToMediaItem(mediaItem2OnGetItem));
                }
            }
        });
    }
        ...
        try {
            ...
            if (i <= 0 || i2 <= 0) {
                ...
                this.mLibrarySession.getCallbackExecutor().execute(new Runnable() { // from class: android.support.v4.media.MediaLibraryService2LegacyStub.3
                    @Override // java.lang.Runnable
                    public void run() {
                        List<MediaItem2> listOnGetSearchResult = MediaLibraryService2LegacyStub.this.mLibrarySession.getCallback().onGetSearchResult(MediaLibraryService2LegacyStub.this.mLibrarySession.getInstance(), controller, str, i, i2, bundle);
                        if (listOnGetSearchResult == null) {
                            result.sendResult(null);
                        } else {
                            result.sendResult(MediaUtils2.convertToMediaItemList(listOnGetSearchResult));
                        }
                    }
                });
            }
