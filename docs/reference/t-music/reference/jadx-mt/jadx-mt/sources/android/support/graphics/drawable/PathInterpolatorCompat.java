...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private float[] f168mX;

    /* JADX INFO: renamed from: mY */
    private float[] f169mY;
    ...
    private void initPath(Path path) {
        ...
        this.f168mX = new float[iMin];
        this.f169mY = new float[iMin];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < iMin; i2++) {
            pathMeasure.getPosTan((i2 * length) / (iMin - 1), fArr, null);
            this.f168mX[i2] = fArr[0];
            this.f169mY[i2] = fArr[1];
        }
        if (Math.abs(this.f168mX[0]) <= 1.0E-5d && Math.abs(this.f169mY[0]) <= 1.0E-5d) {
            int i3 = iMin - 1;
            if (Math.abs(this.f168mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f169mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < iMin) {
                    float[] fArr2 = this.f168mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        sb.append(this.f168mX[0]);
        sb.append(",");
        sb.append(this.f169mY[0]);
        sb.append(" end:");
        int i6 = iMin - 1;
        sb.append(this.f168mX[i6]);
        sb.append(",");
        sb.append(this.f169mY[i6]);
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f168mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f168mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f168mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f169mY[i];
        }
        float f3 = (f - fArr[i]) / f2;
        float[] fArr2 = this.f169mY;
        float f4 = fArr2[i];
