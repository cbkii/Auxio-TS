...
class MediaUtils2 {
    ...
    static MediaBrowserCompat.MediaItem convertToMediaItem(MediaItem2 mediaItem2) {
        ...
        if (metadata == null) {
            mediaDescriptionCompatBuild = new MediaDescriptionCompat.Builder().setMediaId(mediaItem2.getMediaId()).build();
        } else {
        ...
        return new MediaBrowserCompat.MediaItem(mediaDescriptionCompatBuild, mediaItem2.getFlags());
    }
    ...
    static List<MediaItem2> convertToMediaItem2List(Parcelable[] parcelableArr) {
        ...
        if (parcelableArr != null) {
            for (int i = 0; i < parcelableArr.length; i++) {
                if ((parcelableArr[i] instanceof Bundle) && (mediaItem2FromBundle = MediaItem2.fromBundle((Bundle) parcelableArr[i])) != null) {
    ...
    static MediaMetadataCompat convertToMediaMetadataCompat(MediaMetadata2 mediaMetadata2) {
        ...
        for (String str2 : arrayList) {
            ...
            if (obj2 instanceof Float) {
                mediaMetadataCompatBuild.getBundle().putFloat(str2, ((Float) obj2).floatValue());
            } else if (MediaMetadata2.METADATA_KEY_EXTRAS.equals(obj2)) {
                mediaMetadataCompatBuild.getBundle().putBundle(str2, (Bundle) obj2);
            }
        }
    ...
    static List<MediaSession2.CommandButton> convertToCommandButtonList(Parcelable[] parcelableArr) {
        ...
        ArrayList arrayList = new ArrayList();
        for (int i = 0; i < parcelableArr.length; i++) {
            if ((parcelableArr[i] instanceof Bundle) && (commandButtonFromBundle = MediaSession2.CommandButton.fromBundle((Bundle) parcelableArr[i])) != null) {
