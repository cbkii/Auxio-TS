...
public class Grammar {
    ...
    public void expandInPlace() {
        ...
        if ((indexedVector != null && indexedVector.getElement("importVocab") == null) || this.options == null) {
            ...
            StringBuilder m5a = C0000a.m5a(pathToFile);
            m5a.append(superGrammar.exportVocab);
            m5a.append(CodeGenerator.TokenTypesFileSuffix);
            m5a.append(CodeGenerator.TokenTypesFileExt);
            String sb = m5a.toString();
            String fileMinusPath = this.antlrTool.fileMinusPath(sb);
            StringBuilder m5a2 = C0000a.m5a(".");
            m5a2.append(System.getProperty("file.separator"));
            if (!pathToFile.equals(m5a2.toString())) {
                try {
                    this.antlrTool.copyFile(sb, fileMinusPath);
                } catch (IOException unused) {
                    this.antlrTool.toolError("cannot find/copy importVocab file " + sb);
                    return;
                }
            }
        }
    ...
    public void inherit(Rule rule, Grammar grammar) {
        ...
        m5a.append(getName());
        m5a.append(".");
        m5a.append(rule2.getName());
        m5a.append(" has different signature than ");
        m5a.append(grammar.getName());
        m5a.append(".");
        m5a.append(rule2.getName());
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.rules.size(); i++) {
            ...
            if (!getName().equals(rule.enclosingGrammar.getName())) {
                StringBuilder m5a2 = C0000a.m5a("// inherited from grammar ");
                m5a2.append(rule.enclosingGrammar.getName());
