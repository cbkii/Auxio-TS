...
public class DefineGrammarSymbols implements ANTLRGrammarParseBehavior {
    ...
    public void endOptions() {
        ...
        if (grammar2.exportVocab == null && grammar2.importVocab != null) {
            ...
            if (grammar3.importVocab.equals(grammar3.exportVocab)) {
                ...
                m5a.append(" cannot have importVocab same as default output vocab (grammar name); ignored.");
                tool.warning(m5a.toString());
        ...
        if (str.equals(str3)) {
            ...
            StringBuilder m5a2 = C0000a.m5a("exportVocab of ");
            m5a2.append(this.grammar.exportVocab);
            m5a2.append(" same as importVocab; probably not what you want");
    ...
    public void setFileOption(Token token, Token token2, String str) {
        ...
        if (token.getText().equals(IjkMediaMeta.IJKM_KEY_LANGUAGE)) {
            if (token2.getType() == 6) {
                ...
                m0a = "language option must be string or identifier";
            }
            this.language = text;
            return;
        }
        if (token.getText().equals("mangleLiteralPrefix")) {
            ...
            if (type == 6) {
                ...
                line = token2.getLine();
                column = token2.getColumn();
                m0a = "mangleLiteralPrefix option must be string";
            }
        } else {
            ...
            tool3 = this.tool;
            m0a = C0000a.m0a(token, C0000a.m5a("Invalid file-level option: "));
            line = token.getLine();
            column = token2.getColumn();
        }
        tool3.error(m0a, str, line, column);
    }
    ...
    public void startLexer(String str, Token token, String str2, String str3) {
        ...
        if (this.numLexers > 0) {
            Tool tool = this.tool;
            StringBuilder m5a2 = C0000a.m5a("You may only have one lexer per grammar file: class ");
            m5a2.append(token.getText());
            tool.panic(m5a2.toString());
        }
        this.numLexers++;
        reset();
    ...
    public void startParser(String str, Token token, String str2, String str3) {
        ...
        if (this.numParsers > 0) {
            Tool tool = this.tool;
            StringBuilder m5a2 = C0000a.m5a("You may only have one parser per grammar file: class ");
            m5a2.append(token.getText());
            tool.panic(m5a2.toString());
        }
        this.numParsers++;
    ...
    public void startTreeWalker(String str, Token token, String str2, String str3) {
        ...
        if (this.numTreeParsers > 0) {
            Tool tool = this.tool;
            StringBuilder m5a2 = C0000a.m5a("You may only have one tree parser per grammar file: class ");
            m5a2.append(token.getText());
            tool.panic(m5a2.toString());
        }
        this.numTreeParsers++;
