...
The canonical safety lane also runs a comment-aware scan across every production Java/Kotlin source and syntax-checks shell scripts under `scripts/` and `tools/ts18-root-storage-fastpath/`; build, unit-test, lint, formatting and API 29 checks remain separate canonical CI jobs.
