...
| `TYPE_APPLICATION_OVERLAY`       | https://developer.android.com/reference/android/view/WindowManager.LayoutParams#TYPE_APPLICATION_OVERLAY | Correct non-system overlay window type for floating car controls on API 26+; Android still layers it below critical system windows such as the status bar/IME on stock builds. |
| `FLAG_LAYOUT_IN_SCREEN` / `FLAG_LAYOUT_NO_LIMITS` | https://developer.android.com/reference/android/view/WindowManager.LayoutParams | Public flags used by the Topway floating controls to request full-screen/no-limits placement so `y = 0` means the physical display top edge rather than the app-safe area. |
